#!/usr/bin/env python3
"""
SynSLP AnyText2 Controlled A/B Experiment: Baseline vs Font Mimic

Target: Test whether native Font Mimic / font-hint conditioning materially
improves style preservation when editing tight-crop SLP "东泰168" to "苏航268".

Conditions:
- A. Baseline:   Native AnyText2 edit with font_hint_image=[None]*5, font_hint_mask=[None]*5
- B. Font Mimic: Native AnyText2 edit with font_hint_image populated from reference image,
                 and font_hint_mask populated from user mask.

All other parameters (seed, steps, CFG, prompts, strength=1.0, resolution) strictly identical.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="AnyText2 Baseline vs Font Mimic A/B")
    parser.add_argument(
        "--ref_image",
        type=str,
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/mnt/data/zyx/SynSLP/outputs/font_mimic_ab/dongtai168",
    )
...
def main():
    ...
    # Fixed base parameters across both conditions
    fixed_base_params = {
        "mode": "edit",
        "sort_priority": "↔",
        "show_debug": False,
    ...
    print("\n" + "=" * 60)
    print("Step 3: Running A/B Conditions")
    print(f"Target Text:    \"{args.target_text}\"")
    print(f"Fixed Seed:     {args.seed}")
    print(f"Fixed Steps:    {args.ddim_steps}")
    print(f"Fixed Strength: {args.strength}")
    print(f"Fixed CFG:      {args.cfg_scale}")
    print("=" * 60)

    # Condition A: Baseline (Font Mimic disabled)
    print("\n---> Running Condition A: Baseline (Font Mimic disabled) ...")
    params_a = dict(fixed_base_params)
    params_a["font_hint_image"] = [None] * 5
    params_a["font_hint_mask"] = [None] * 5

    t0 = time.time()
    with torch.no_grad():
        results_a, code_a, warn_a, _ = model(input_data, **params_a)
    t_a = time.time() - t0
    if code_a < 0:
        raise RuntimeError(f"Baseline inference failed: {warn_a}")

    out_a_512 = results_a[0][..., ::-1]
    path_a_512 = output_dir / "baseline.png"
    cv2.imwrite(str(path_a_512), out_a_512)

    out_a_orig = cv2.resize(out_a_512, (orig_w, orig_h), interpolation=cv2.INTER_LANCZOS4)
    path_a_orig = output_dir / "baseline_orig_res.png"
    cv2.imwrite(str(path_a_orig), out_a_orig)
    print(f"PASS: Baseline generated in {t_a:.2f}s")
    print(f"Saved: {path_a_512} ({render_w}x{render_h})")
    print(f"Saved: {path_a_orig} ({orig_w}x{orig_h})")

    # Condition B: Font Mimic (Font hint enabled from reference and mask)
    print("\n---> Running Condition B: Font Mimic (Font hint enabled) ...")
    params_b = dict(fixed_base_params)
    # Native API expects font_hint_image to be RGB array (H, W, 3)
    # and font_hint_mask to be single-channel uint8 array (H, W, 1) with values > 0
    params_b["font_hint_image"] = [ref_rgb_scaled, None, None, None, None]
    params_b["font_hint_mask"] = [mask_scaled[..., 0:1], None, None, None, None]

    t0 = time.time()
    with torch.no_grad():
        results_b, code_b, warn_b, _ = model(input_data, **params_b)
    t_b = time.time() - t0
    if code_b < 0:
        raise RuntimeError(f"Font Mimic inference failed: {warn_b}")

    out_b_512 = results_b[0][..., ::-1]
    path_b_512 = output_dir / "font_mimic.png"
    cv2.imwrite(str(path_b_512), out_b_512)

    out_b_orig = cv2.resize(out_b_512, (orig_w, orig_h), interpolation=cv2.INTER_LANCZOS4)
    path_b_orig = output_dir / "font_mimic_orig_res.png"
    cv2.imwrite(str(path_b_orig), out_b_orig)
    print(f"PASS: Font Mimic generated in {t_b:.2f}s")
    print(f"Saved: {path_b_512} ({render_w}x{render_h})")
    print(f"Saved: {path_b_orig} ({orig_w}x{orig_h})")
    ...
    comparison_items_512 = [
        ("Reference: 东泰168", ref_bgr),
        (f"A. Baseline: 苏航268 (Elapsed: {t_a:.2f}s)", out_a_512),
        (f"B. Font Mimic: 苏航268 (Elapsed: {t_b:.2f}s)", out_b_512),
    ]
    grid_path_512 = output_dir / "comparison.png"
    create_comparison_grid(comparison_items_512, grid_path_512, (render_w, render_h))

    comparison_items_orig = [
        ("Reference: 东泰168", ref_bgr),
        (f"A. Baseline: 苏航268 (Elapsed: {t_a:.2f}s)", out_a_orig),
        (f"B. Font Mimic: 苏航268 (Elapsed: {t_b:.2f}s)", out_b_orig),
    ]
    grid_path_orig = output_dir / "comparison_orig_res.png"
    create_comparison_grid(comparison_items_orig, grid_path_orig, (orig_w, orig_h))
    ...
    print(f"A (Baseline):      {t_a:.2f}s -> {path_a_512}")
    print(f"B (Font Mimic):    {t_b:.2f}s -> {path_b_512}")
    print(f"Comparison Grid:   {grid_path_512}")
    print("A/B EXPERIMENT COMPLETED SUCCESSFULLY")
